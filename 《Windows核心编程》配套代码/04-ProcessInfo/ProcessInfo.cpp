...
VOID Dlg_PopulateProcessList(HWND hwnd) {
   ...
   SetWindowRedraw(hwndList, FALSE);//((void)::SendMessage(hwndList, WM_SETREDRAW, (WPARAM)(BOOL)(FALSE), 0L))
   ComboBox_ResetContent(hwndList); //清空COMBOBOX
   ...
   //(void)(SendMessage)((hwnd), WM_COMMAND, 
   //   MAKEWPARAM((UINT)(IDC_PROCESSMODULELIST),(UINT)(CBN_SELCHANGE)), (LPARAM)(HWND)(hwndList))
...
VOID Dlg_PopulateModuleList(HWND hwnd) {
   ...
   SendMessage(hwndModuleHelp, WM_SETREDRAW, 0L, 0L);
   ...
   for (; fOk; fOk = thProcesses.ProcessNext(&pe)) {
      ...
      for (; fOk; fOk = thModules.ModuleNext(&me)) {
		...
		//((int)(DWORD)SendMessage((hwndModuleHelp), LB_FINDSTRINGEXACT, 
   ...
   for (int i = 0; i < nNumModules; i++) {
	  ...
	  //int nIndex = ComboBox_AddString(hwndList, sz);
...
VOID ShowProcessInfo(HWND hwnd, DWORD dwProcessID) {
   ...
   for (; fOk; fOk = th.ProcessNext(&pe)) {
      if (pe.th32ProcessID == dwProcessID) {
         ...
         AddText(hwnd, TEXT("   PID=0x%08X, ParentPID=0x%08X, ")
   ...
   AddText(hwnd, TEXT("\r\nModules Information:\r\n")
      ...
      cchAddress, TEXT("ImagAddr"), TEXT("Size"));//cchAddress指明BaseAddr和ImagAddr的宽度，即*的值
...
BOOL Dlg_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam) {
   ...
   //SetWindowFont(GetDlgItem(hwnd, IDC_RESULTS), 
      //GetStockFont(SYSTEM_FIXED_FONT), FALSE);
   SendMessage(GetDlgItem(hwnd, IDC_PROCESSMODULELIST), WM_SETFONT, (WPARAM)(GetStockFont(SYSTEM_FIXED_FONT)), (LPARAM)(FALSE));

   

   
   SendMessage(GetDlgItem(hwnd, IDC_RESULTS), WM_SETFONT, (WPARAM)(GetStockFont(DEVICE_DEFAULT_FONT)), (LPARAM)(FALSE));
   SendMessage(hwnd, WM_CTLCOLOR, (WPARAM)(GetStockFont(DEVICE_DEFAULT_FONT)), (LPARAM)(FALSE));
   HDC hdc=GetDC(GetDlgItem(hwnd, IDC_RESULTS));
   //SetDCPenColor(hdc,RGB(0,0,0xFF));
   //SelectObject(hdc,GetStockObject(DC_PEN));
   SetBkMode(hdc,TRANSPARENT);
   SetTextColor(hdc,RGB(0,0,255));
   RedrawWindow(GetDlgItem(hwnd, IDC_RESULTS),NULL,NULL,NULL);
   //SendMessage(GetDlgItem(hwnd, IDC_RESULTS), WM_SETREDRAW, (WPARAM)(TRUE), (LPARAM)(0L));
