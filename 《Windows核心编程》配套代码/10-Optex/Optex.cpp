...
PSTR COptex::ConstructObjectName(PSTR pszResult, PCSTR pszPrefix, BOOL fUnicode, PVOID pszName)
{
   pszResult[0] = 0;
   if (pszName == NULL) 
      return(NULL);
...
void COptex::CommonConstructor(DWORD dwSpinCount, BOOL fUnicode, PVOID pszName) 
{
   ...
   if (pszName == NULL) 
   {  // Creating a single-process optex
   ...
   {  // Creating a cross-process optex
...
COptex::~COptex() 
{
   ...
   if (!IsSingleProcessOptex() && (m_psi->m_dwThreadId == GetCurrentThreadId())) 
   {
