...
BOOL Dlg_OnInitDialog(HWND hwnd, HWND hwndFocus, LPARAM lParam) 
{
   ...
   for (int n = 0; n <= 1; n++) 
   {
      ...
      Edit_LimitText(GetDlgItem(hwnd, (n == 0) ? IDC_WINDOW0TEXT : IDC_WINDOW1TEXT), g_nChars);
   }
...
void Dlg_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify) 
{
   ...
   case IDC_WINDOW1STORAGE:
      if (codeNotify == CBN_SELCHANGE) 
	  {
         ...
         HWND hwndText = GetDlgItem(hwnd, ((nWindow == 0) ? IDC_WINDOW0TEXT : IDC_WINDOW1TEXT));
         ...
         Edit_SetText(hwndText, IsWindowEnabled(hwndText) ? (PCTSTR) (PVOID) g_aw[nWindow] : TEXT("(No storage)"));
      }
      break;
