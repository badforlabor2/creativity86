...
public:
   ...
   BOOL Create(SIZE_T dwBytes, PVOID pvPreferredWindowBase = NULL) 
   {
      ...
      m_pvWindow = VirtualAlloc(pvPreferredWindowBase, dwBytes, MEM_RESERVE | MEM_PHYSICAL, PAGE_READWRITE);
   ...
   BOOL UnmapStorage() 
   {
      ...
      return(MapUserPhysicalPages(m_pvWindow, mbi.RegionSize / sm_sinf.dwPageSize, NULL));
   }
...
public:
   ...
   BOOL Allocate(ULONG_PTR ulBytes) 
   {
      ...
      m_pulUserPfnArray = (PULONG_PTR)HeapAlloc(GetProcessHeap(), 0, m_ulPages * sizeof(ULONG_PTR));
      ...
      if (fOk) 
	  {
         ...
         fOk = AllocateUserPhysicalPages(GetCurrentProcess(), &m_ulPages, m_pulUserPfnArray);
   ...
   BOOL Free() 
   {
	  ...
	  {
         fOk = FreeUserPhysicalPages(GetCurrentProcess(), &m_ulPages, m_pulUserPfnArray);
         if (fOk) 
		 {
