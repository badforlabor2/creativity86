...
int main()
{
	...
	FILE* rfile = fopen("kfxitem.csv", "rb");
	if(rfile)
	{
		...
		readed = fread(data, 1, blocksize, rfile);
		
		//格式转换
		Utf8_16_Read convReader;
		Utf8_16_Write convWriter;
		readed = convReader.convert(data, readed);
		cout<<"file encode:"<<convReader.getEncoding()<<endl;
		const char* newData;
		if(convReader.getEncoding() == uni8Bit
				|| convReader.getEncoding() == uni7Bit)
		{
			newData = WcharMbcsConvertor::getInstance()->encode(CP_ACP, CP_UTF8, data);
		}
		else
		{
			newData = data;
		...
		//现在newData是utf8编码，然后进行编码转化
		//如果目标编码是ANSI的，那么
		//newData = WcharMbcsConvertor::getInstance()->encode(CP_UTF8, CP_ACP, newData);

		convWriter.fopen(TEXT("cp-123.txt"), TEXT("wb"));
		//convWriter.setEncoding(uni16BE);
		convWriter.setEncoding(uniCookie);
		convWriter.fwrite(newData, readed);
		convWriter.fclose();
	}
